...
from tube.functions.utils import title, sign_color, table
...
class CompareMeasurementAndPrint(Block):
    ...
    def datasets_difference(self, dataset1: Dataset, dataset2: Dataset) -> float:
        return (
            dataset1.payload["measurements"][self.measurement] -
            dataset2.payload["measurements"][self.measurement]
        )
    ...
    def execute(self, datasets: Dict[str, Dataset]) -> Dict[str, Dataset]:
        ...
        print(title("\nAbsolute difference in %s" % self.measurement))
        print(table(output))
        return datasets
